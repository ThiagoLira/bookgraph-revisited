...
INPUT_PATH="${1:-$REPO_ROOT/test_book_subset.txt}"
CHUNK_SIZE="${2:-50}"
...
echo "Starting GPU monitor (GPU 0) -> $GPU_LOG" >&2
bash "$MONITOR_SCRIPT" "$GPU_LOG" "$INTERVAL" 0 &
MONITOR_PID=$!
...
set title "GPU 0 Utilization - $(basename "$INPUT_PATH") (${TOTAL_SECONDS}s, ctx:${CONTEXT_PER_REQUEST}, conc:${MAX_CONCURRENCY}, chunk:${CHUNK_SIZE})"
set xdata time
...
set yrange [0:100]
set grid
plot "$GPU_LOG" using 1:2 with lines title "GPU 0"
EOF
