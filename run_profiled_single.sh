...
if [[ $# -gt 0 ]] && [[ "$1" == "-h" || "$1" == "--help" ]]; then
  echo "Usage: $0 [INPUT_TXT] [CHUNK_SIZE] [MAX_CONCURRENCY] [MAX_INPUT_TOKENS] [MAX_COMPLETION_TOKENS] [MODEL_PATH] [GPU_LAYERS] [SERVER_BINARY] [INTERVAL]" >&2
  echo "" >&2
...
# In llama.cpp, context is divided among parallel slots: effective_ctx_per_slot = total_ctx / np
# So we need: total_ctx = (MAX_INPUT_TOKENS + MAX_COMPLETION_TOKENS) * MAX_CONCURRENCY
CONTEXT_PER_REQUEST=$((MAX_INPUT_TOKENS + MAX_COMPLETION_TOKENS))
CONTEXT_SIZE=$((CONTEXT_PER_REQUEST * MAX_CONCURRENCY))
...
echo "  Total context size (-c): $CONTEXT_SIZE" >&2
echo "  Per-request context: $CONTEXT_PER_REQUEST (input: $MAX_INPUT_TOKENS + completion: $MAX_COMPLETION_TOKENS)" >&2
...
set title "GPU 1 Utilization - $(basename "$INPUT_PATH") (${TOTAL_SECONDS}s, ctx:${CONTEXT_SIZE}, conc:${MAX_CONCURRENCY}, chunk:${CHUNK_SIZE})"
set xdata time
